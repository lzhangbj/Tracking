...
def parse_vid_img_rec(filename, classhash, img_ids, defaultIOUthr=0.5, pixelTolerance=10):
    """
...
def vid_img_eval(detpath, annopath, imageset_file, classname_map, annocache, ovthresh=0.5):
    """
    ...
    if not os.path.isfile(annocache):
        ...
        for ind, image_filename in enumerate(img_basenames):
            recs.append(parse_vid_img_rec(annopath.format('VID/' + image_filename), classhash, gt_img_ids[ind]))
            if ind % 100 == 0:
    ...
    # recs = np.zeros(len(classname_map))
    # precs = np.zeros(len(classname_map))
