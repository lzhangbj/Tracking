...
#       '''
#           input channel:  inplanes
#           output channel: 4*planes
#           blocks means how many repititions of block
#       '''
#       downsample = None
#       if stride != 1 or self.inplanes != planes * block.expansion:
#           downsample = nn.Sequential(
#               nn.Conv2d(self.inplanes, planes * block.expansion, # 64, 1024, 
...
class resnet(_fasterRCNN):
	...
	def _init_modules(self):
		...
		self.RCNN_top.apply(set_bn_fix)         # zl why set bn fixed
...
class resnet_rfcn(_rfcn):
	def __init__(self, classes, num_layers=101, pretrained=False, class_agnostic=False):
		self.model_path = 'data/pretrained_model/resnet101_caffe.pth'
		self.dout_base_model = 512 #1024
		self.dout_head_model = 2048
		self.pretrained = pretrained
		self.class_agnostic = class_agnostic
	...
	def _init_modules(self):
		...
		# self.RCNN_top = nn.Sequential(resnet.layer4)  # output (batch, 2048)
